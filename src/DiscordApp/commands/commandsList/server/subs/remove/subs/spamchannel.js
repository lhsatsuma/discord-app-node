module.exports = {
	name: 'spamchannel',
	description: '[ADMIN] Add new spam channel to server list',
	deleteMsgOnSuccess: true,
	deleteMsgOnError: true,
	usage: () => {
		return global.bot_prefix+'server add spamchannel <guild_id> <spam_channel>'
	},
	allowedChannels: [
		'774240024723193856',
	],
	async execute(message, args) {
        ...
        if(!!args[1]){
			...
			if(!!serversList[args[0]]){
				if(serversList[args[0]]['spam_channels'].indexOf(args[1]) != -1){
					resultReturn.err = `Ops! O servidor já possui este canal cadastrado!
					ID: ${args[0]}
					CANAL: ${args[1]}`;
				}else{
					serversList[args[0]]['spam_channels'].push(args[1]);
					...
					.setTitle('[ADMIN] Add new spam channel to server list')
					.setDescription(`**Novo spam Channel adicionado com sucesso!**
